...
function handleTofuOperations(workingDirectory) {
    ...
    // Initialize tofu
    const init = spawnSync('docker', ['run', '-v', `${workingDirectory}:/app`, 'ghcr.io/opentofu/opentofu:latest', 'init'], { stdio: 'inherit' });
    if (init.error) {
    ...
    // Check for drift
    const plan = spawnSync('docker', ['run', '-v', `${workingDirectory}:/app`, 'ghcr.io/opentofu/opentofu:latest', 'plan', '-detailed-exitcode'], { stdio: 'inherit' });
    if (plan.error) {
