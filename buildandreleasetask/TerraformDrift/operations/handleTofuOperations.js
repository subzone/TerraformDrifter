...
function handleTofuOperations(workingDirectory) {
    ...
    // Initialize tofu
    const init = spawnSync('docker', ['run', `--workdir=${workingDirectory}`, `source=${workingDirectory}`, `target=${workingDirectory}`, 'ghcr.io/opentofu/opentofu:latest', 'init'], { stdio: 'inherit' });
    if (init.error) {
    ...
    // Check for drift
    const plan = spawnSync('docker', ['run', `--workdir=${workingDirectory}`, `\source=${workingDirectory}`, `${workingDirectory}`, 'ghcr.io/opentofu/opentofu:latest', 'plan', '-detailed-exitcode'], { stdio: 'inherit' });
    if (plan.error) {
